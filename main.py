...
def uketuke():
    try:
        ...
        tikokuhour = int(config["other_setting"]["delay_standard_hour"])
        tikokuhour = str(tikokuhour)
        tikokuminute = int(config["other_setting"]["delay_standard_minute"])
...
def setting():
    ...
    tk.Label(setting_window, text="遅刻時刻(時：分)", font=("MSゴシック", 12)).grid(column=0, row=7)
    delay_hour_config = tk.StringVar()
    delay_hour_config.set(delay_hour)
    delay_hour_Entry = tk.Entry(setting_window, width=5, textvariable=delay_hour_config).grid(column=1, row=7)
    delay_minute_config = tk.StringVar()
    delay_minute_config.set(delay_minute)
    delay_minute_Entry = tk.Entry(setting_window, width=5, textvariable=delay_minute_config).grid(column=2, row=7)
    tk.Label(setting_window, text="記録文字", font=("MSゴシック", 12)).grid(column=0, row=8)
    ...
    record_character_Entry = tk.Entry(setting_window, width=15, textvariable=record_character_config).grid(column=1,
                                                                                                           columnspan=2,
                                                                                                           row=8)
    tk.Label(setting_window, text="syscode", font=("MSゴシック", 12)).grid(column=0, row=9)
    syscode_config = tk.StringVar()
    syscode_config.set(syscode_code)
    syscode_Entry = tk.Entry(setting_window, width=15, textvariable=syscode_config).grid(column=1, columnspan=2, row=9)
    tk.Label(setting_window, text="servicecode", font=("MSゴシック", 12)).grid(column=0, row=10)
    ...
    syscode_Entry = tk.Entry(setting_window, width=15, textvariable=servicecode_config).grid(column=1, columnspan=2,
                                                                                             row=10)
    tk.Label(setting_window, text="待機時間[秒]", font=("MSゴシック", 12)).grid(column=0, row=11)
    ...
    standby_time_Entry = tk.Entry(setting_window, width=15, textvariable=standby_time_config).grid(column=1, columnspan=2,
                                                                                               row=11)
    tk.Label(setting_window, text="通常Excelパス", font=("MSゴシック", 12)).grid(column=0, row=12)
    ...
    excel_path_Entry = tk.Entry(setting_window, width=15, textvariable=excel_path_config).grid(column=1, columnspan=2,
                                                                                               row=12)
    tk.Label(setting_window, text="名簿Excelパス", font=("MSゴシック", 12)).grid(column=0, row=13)
    ...
    tk.Button(setting_window, text="保存",
              command=lambda: config_write(delay_config, UnReginstered_config, Rewriting_config, detail_print_config,
                                           touch_voice_config, add_recording_config, delay_hour_config,
                                           ...
                                           more_delay_config), font=("MSゴシック", 15)).grid(column=0, row=14, columnspan=7)


def config_write(delay_config, UnReginstered_config, Rewriting_config, detail_print_config, touch_voice_config,
                 add_recording_config, delay_hour_config, delay_minute_config, record_character_config,
...
radiobutton1.grid(column=5, row=1, sticky=tk.W)
radiobutton2 = tk.Radiobutton(frame1, text="リセット動作無し", variable=start_option, value=2)
radiobutton2.grid(column=5, row=2, sticky=tk.W)
